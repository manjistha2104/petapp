...
import CategoriesComponent from "../components/CategoriesComponent"; // Adjust the path if needed
...
const BannerPage = () => {
  ...
  const saveBannerSettings = async () => {
    try {
      ...
      const response = await fetch("/api/homecategories", {
        method: "POST",
        body: formData,
      });
    ...
    <div className="pt-5">
      ...
      {popupVisible && (
        <div className="fixed bottom-5 right-5 flex items-center justify-between rounded bg-gray-800 p-3 text-white shadow-lg">
          <span>categories settings saved successfully!</span>
          <button
