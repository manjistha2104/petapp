...
export async function POST(req) {
  ...
  try {
    ...
    let existingBannerCollection = await HomeservicesModel.findOne({
      title: bannerTitle,
    });
    const existingBanners = existingBannerCollection?.banners || [];

    let fileIndex = 0;
    const uploadDir = "/var/www/petapp/uploads/homeservice";
    ...
    for (let i = 0; i < uniqueNames.length; i++) {
      ...
      const imageUrl = `https://www.causeforpaws.in/uploads/homeservice/${bannerName}-${file.name}`;
    ...
    if (existingBannerCollection) {
      ...
      const newBannerCollection = new HomeservicesModel({
        title: bannerTitle,
...
export async function GET(req) {
  try {
    const banners = await HomeservicesModel.find();
...
export async function DELETE(req) {
  try {
    ...
    const bannerCollection = await HomeservicesModel.findOne({
    ...
    const bannerToDelete = bannerCollection.banners.find(
    ...
    const filePath = path.join(
      "/var/www/petapp/uploads",
      path.basename(bannerToDelete.imageUrl)
    );
    ...
    bannerCollection.banners = bannerCollection.banners.filter(
      (banner) => banner.name !== name
    );
