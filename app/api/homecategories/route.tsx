...
import homecategoriesModel from "@/models/homecategories";
...
export async function POST(req) {
  ...
  try {
    ...
    const bannerTitle = "Homepage Categories";

    let existingBannerCollection = await homecategoriesModel.findOne({ title: bannerTitle });
    const existingBanners = existingBannerCollection?.banners || [];

    let fileIndex = 0;
    const uploadDir = "/var/www/petapp/uploads/homecategories";
    ...
    for (let i = 0; i < uniqueNames.length; i++) {
      ...
      const imageUrl = `https://www.causeforpaws.in/uploads/homecategories/${bannerName}-${file.name}`;
    ...
    if (existingBannerCollection) {
      ...
      const newBannerCollection = new homecategoriesModel({
        title: bannerTitle,
...
export async function GET(req) {
  try {
    const banners = await homecategoriesModel.find();
